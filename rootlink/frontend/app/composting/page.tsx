    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-8 py-12">
      ...
      {showForm && (
        <Card variant="plain" className="p-6 mb-6 space-y-4">
          <h3 className="font-display font-bold text-stone-800">{t("waste.create_hub")}</h3>
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">{t("waste.hub_name")}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">{t("waste.hub_location")}</label>
            ...
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">{t("waste.hub_description")}</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={2}
                className="w-full px-3 py-2 rounded-xl border border-primary-100 bg-white text-sm focus:border-primary-400 focus:ring-2 focus:ring-primary-500/15" />
            </div>
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">{t("waste.capacity_week")} (kg)</label>
                <input type="number" step="0.1" value={capacity} onChange={(e) => setCapacity(e.target.value)}
                  className="w-full px-3 py-2 rounded-xl border border-primary-100 bg-white text-sm focus:border-primary-400 focus:ring-2 focus:ring-primary-500/15" />
              </div>
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">{t("waste.operating_hours")}</label>
            ...
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-2">{t("waste.accepted_materials")}</label>
              <div className="flex flex-wrap gap-2">
                {MATERIALS.map((m) => (
                  <button key={m.value} type="button" onClick={() => toggleMaterial(m.value)}
                    className={`px-3 py-1.5 text-xs rounded-lg border transition ${
                      materials.includes(m.value) ? "bg-green-600 text-white border-green-600" : "bg-white text-stone-600 border-stone-200 hover:border-green-300"
                    }`}>
                    {t(m.labelKey)}
                  </button>
                ))}
            ...
            <Card key={hub.id} variant="default" className="p-5">
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-display font-semibold text-stone-800 truncate">{hub.name}</h3>
              ...
              {hub.accepted_materials?.length > 0 && (
                <div className="flex flex-wrap gap-1 mb-3">
                  {hub.accepted_materials.slice(0, 3).map((m: string) => (
                    <span key={m} className="text-[10px] bg-green-50 text-green-700 px-2 py-0.5 rounded-full">
                      {t(`waste.material_${m}`) || m}
                    </span>
                  ))}
                  {hub.accepted_materials.length > 3 && <span className="text-[10px] text-stone-400">+{hub.accepted_materials.length - 3}</span>}
                </div>
              )}

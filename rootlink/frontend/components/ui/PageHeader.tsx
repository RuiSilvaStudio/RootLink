...
export function PageHeader({ icon, badge, title, description, subtitle, action, children, className = "" }: Props) {
  ...
  return (
    <header className={`max-w-2xl ${className}`}>
      ...
      <h1 className="text-4xl sm:text-5xl md:text-6xl font-display font-semibold text-stone-800 dark:text-stone-100 leading-[1.08] tracking-tight">
        {title}
      </h1>
      <div className="mt-4 w-16 h-0.5 bg-primary-300/60 rounded-full" />
      {desc && (
        <p className="mt-6 text-lg text-stone-500 dark:text-stone-400 leading-relaxed font-serif max-w-xl">
          {desc}
        </p>
      )}
      {action && (
